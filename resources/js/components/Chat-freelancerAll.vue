  ...
  <div class="row">
          ...
          <div class="scroll-y me-n5 pe-5 h-lg-auto" data-kt-scroll="true"
            ...
            <div class="d-flex d-flex__column py-1">
              ...
              <div class="scroll-container candidates-scroll scroll-section">
                <div v-for="user in sortedCandidates" :key="user.user?.id || user.id"
                  class="d-flex flex-column align-items-center" @click.prevent="selectUser(user.user)">
                  <div class="user__details" :class="{
                    'user-active': selectedUser?.id === user.user?.id,
                  }">
                    <div class="symbol symbol-45px symbol-circle">
                      <img :src="user.profile_image ? getImageFileUrl(user.profile_image) : defaultImage"
              ...
              <div class="scroll-container contributors-scroll scroll-section">
                <div v-for="user in sortedContributors" :key="user.user?.id || user.id"
                  class="d-flex flex-column align-items-center" @click.prevent="selectContributor(user)">
                  <div class="user__details" :class="{
                    'user-active': selectedContributor?.user?.id === user.user?.id,
                    }">
                    <div class="symbol symbol-45px symbol-circle">
                      <img :src="user.user.profile_image ? getImageFileUrl(user.user.profile_image) : defaultImage"
                        alt="Profile Image" class="img-fluid rounded-circle shadow-sm"
                        style="width: 60px; height: 60px" />
                    </div>
                    <div class="ms-5">
    ...
    <div class="col-lg-7">
      <div class="card h-100">
        ...
        <div class="card-body chat-box chat-box__contributor" id="chatBox__freelancerAll">
          <div v-for="msg in sortedMessages" :key="msg.id"
            ...
            <div :class="msg.user_id === currentUserId ? 'text-end' : 'text-start'">
              ...
              <div :class="msg.user_id === currentUserId
                  ...
                  : 'p-3 rounded bg-light text-dark d-inline-block'" v-if="msg.text">
                {{ msg.text }}
              </div>
              ...
              <div v-if="msg.file_path" style="margin-top: 10px">
                ...
                <div v-else-if="getFileDisplayType(msg.file_type) === 'pdf'">
                  <a :class="msg.user_id === currentUserId
                      ? 'p-3 rounded bg-primary text-white d-inline-block'
                      : 'p-3 rounded bg-light text-dark d-inline-block'" :href="getImageFileUrl(msg.file_path)"
                    target="_blank">
                    📄 View PDF
                  </a>
                </div>

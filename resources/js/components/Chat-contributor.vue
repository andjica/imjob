              ...
              <div v-for="recruiter in uniqueRecruiters" :key="recruiter.user_id">
                <div v-if="recruiter && recruiter.user_id" class="d-flex align-items-center px-2 user-card"
                  ...
                  <div class="ms-5">
                    ...
                    <small><i>{{ recruiter.is_freelancer === 0 ? 'Freelancer' : 'Recruiter' }}</i></small>
                  </div>
                </div>
                <div v-else>
    ...
    <div class="col-lg-7">
      <div class="card card-flush h-100">
        <div class="card-header">
          <h3 class="card-title">
            Chat with {{ selectedUser ? selectedUser.first_name : 'Select a user' }}
          </h3>
        </div>
        <div class="card-body chat-box" id="chatBox">
          <div v-for="msg in messages" :key="msg.id" :class="['chat-message', msg.user_id === currentUserId ? 'sent' : 'received']">
            <div :class="msg.user_id === currentUserId ? 'text-end' : 'text-start'">
              <div :class="msg.user_id === currentUserId ? 'p-3 rounded bg-primary text-white d-inline-block' : 'p-3 rounded bg-light text-dark d-inline-block'">
                {{ msg.text }}
              </div>
              <div class="mt-1">
        ...
        <div class="card-footer border-top pt-4">
          <form id="chatForm">
            <div class="input-group">
              <input type="text" class="form-control form-control-solid px-13" name="input" placeholder="Type your message..." v-model="message" />
...
export default {
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      const receiverId = this.selectedUser?.id || this.selectedRecruiter?.user?.id;
  ...
  mounted() {
    ...
    this.picker.on("emoji", (emoji) => (this.message += emoji.emoji));
    ...
    Echo.private("chat." + this.currentUserId)
     ...
     .listen("MessageSent", (payload) => {
        ...
        const activeReceiverId = this.selectedUser?.id || this.selectedRecruiter?.user?.id;
